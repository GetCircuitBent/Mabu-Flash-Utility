...
function Wait-DeviceSettled {
    ...
    # transiently (a re-run then passes). Wait for sys.boot_completed, then a fixed
    # settle margin for package/service registration.
    ...
    while ((Get-Date) -lt $deadline) {
        $bc = (& $script:ADB -s $Dev shell getprop sys.boot_completed 2>&1 | Out-String).Trim()
        if ($bc -eq '1') { break }
        Start-Sleep -Seconds 3
    }
...
function Invoke-MabuFlash {
    ...
    try {
        ...
        if (Test-Loader) {
            ...
            if (-not $dev) {
                # Before handing out generic power-cycle advice: the tablet may be
                # sitting right there on the bus with the wrong driver on its adb
                # interface. That looks identical from here (adb sees nothing) but no
                # amount of power-cycling will ever fix it, so name it precisely.
                ...
                if ($mis.Count) {
                    ...
                    # Repair it here rather than sending the operator to Device
                    # Manager. The "Android ADB driver" is WinUSB plus one registry
                    # value, and these nodes are already on WinUSB (that is what
                    # Zadig left behind), so the value is all that is missing --
                    # no INF, no unsigned-driver prompt.
        ...
        if (-not $SkipApps) {
            ...
            if ($testDev) {
                ...
                if ($script:LastSelfTestFails -gt 0) {
                    Warn "Self-test reported $($script:LastSelfTestFails) failure(s) on the first pass;"
                    Warn 'the system may still have been settling. Waiting 20s and re-testing once...'
