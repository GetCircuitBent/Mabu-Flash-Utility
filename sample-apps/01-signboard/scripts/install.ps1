<#
.SYNOPSIS
    Build, install and launch Mabu Signboard on a Mabu over Wi-Fi ADB.
...
.DESCRIPTION
    Does the whole sequence, in the order that actually works:
      ...
      4. Build and install.
      5. Pre-grant the storage permission and create /sdcard/signboard/.
...
Adb shell mkdir -p /sdcard/signboard 2>$null | Out-Null
Ok 'Permission granted, /sdcard/signboard ready'
...
Write-Host ''
Write-Host 'Drop your own sign onto the device with:' -ForegroundColor Gray
Write-Host '  adb push mysign.gif /sdcard/signboard/' -ForegroundColor Gray
Write-Host ''

if ($Logcat) {
    Info 'Tailing logs (Ctrl-C to stop)'
    Adb logcat -c
    Adb logcat MabuSerial:* MabuTween:* MabuIdle:* MabuSign:* MabuControl:* AndroidRuntime:E '*:S'
}
