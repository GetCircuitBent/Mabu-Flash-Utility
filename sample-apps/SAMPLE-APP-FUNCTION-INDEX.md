...
| # | Function | What the Sample Must Show | Sample App |
|---|---|---|---|
| 26 | Track hands | There is no hand-tracking library for this device: MediaPipe dropped `armeabi-v7a`, ML Kit has no hand API, and ML Kit Pose cannot share a 100 ms frame budget with face detection. Motion and background subtraction are out too, because the camera is bolted to a head that moves. What works is chroma-match blob tracking in YCbCr, which the camera hands you for free in NV21. Critically it ships **no** colour constants: the textbook fixed skin thresholds are biased toward light skin, so the match window is measured live from the player's own face, with luminance-adaptive tolerance and manual and marker calibration as fallbacks. A worked example of picking the technique the hardware allows, and of removing a biased step rather than compensating for it. | TBD |
