...
if (Test-Loader) {
    ...
    if (-not $dev) { Fail 'No adb device and no Loader. Power-cycle the tablet to catch Loader, then re-run.'; exit 1 }
...
if ($state -ne 'Liberated') {
    ...
    if ($doWipe) {
        ...
        if (-not (Wait-Loader 30)) { Fail 'Loader not re-caught.'; exit 1 }
        Ok 'Loader re-caught.'
